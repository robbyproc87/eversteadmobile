...
export async function PUT(request: NextRequest) {
  ...
  const rateLimitResult = checkRateLimit(`planner-gratitudes:${user.id}`, 60, 60000);
  ...
  try {
    ...
    const results = await Promise.all(
      items.map((item: { ordinal: number; text?: string }) =>
        prisma.dailyPlanGratitude.upsert({
          where: {
            dailyPlanId_ordinal: {
              dailyPlanId,
    ...
    console.error("Gratitudes upsert error:", error);
    return NextResponse.json({ error: "Failed to upsert gratitudes" }, { status: 500 });
  }
}
