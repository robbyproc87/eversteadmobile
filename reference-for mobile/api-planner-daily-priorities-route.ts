...
export async function PUT(request: NextRequest) {
  ...
  const rateLimitResult = checkRateLimit(`planner-priorities:${user.id}`, 60, 60000);
  ...
  try {
    ...
    const results = await Promise.all(
      items.map((item: { ordinal: number; text?: string }) =>
        prisma.dailyPlanPriority.upsert({
          where: {
            dailyPlanId_ordinal: {
              dailyPlanId,
    ...
    console.error("Priorities upsert error:", error);
    return NextResponse.json({ error: "Failed to upsert priorities" }, { status: 500 });
  }
}
